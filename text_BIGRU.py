...
class TextGRU(object):
    def __init__(self, source_vocab_size, target_vocab_size, buckets, state_size, num_layers, embedding_size, max_gradient,
                 batch_size, learning_rate, forward_only=False, dtype=tf.float32):
        ...
        with tf.variable_scope("seq2seq", dtype=dtype):
            with tf.variable_scope("encoder"):
                ...
                encoder_outputs, encoder_states = tf.nn.bidirectional_dynamic_rnn(encoder_fw_cell, encoder_bw_cell, encoder_inputs_emb,
            ...
            with tf.variable_scope("attention"):
                ...
                decoder_cell = tf.contrib.seq2seq.DynamicAttentionWrapper(decoder_cell, attention, state_size * 2)
                wrapper_state = tf.contrib.seq2seq.DynamicAttentionWrapperState(self.init_state, self.prev_att)

            with tf.variable_scope("decoder") as scope:
                decoder_emb = tf.get_variable("embedding", [target_vocab_size, embedding_size], initializer=emb_init)
                decoder_cell = tf.contrib.rnn.OutputProjectWrapper(decoder_cell, target_vocab_size)
                ...
                if not forward_only:
                    ...
                    st_toks = tf.convert_to_tensor([utils.ID_GO] * batch_size, dtype=tf.int32)
                    helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(decoder_emb, st_toks, utils.ID_EOS)
                    decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cell, helper, wrapper_state)
    ...
    def step(self, session, encoder_inputs, decoder_inputs, encoder_len, decoder_len, forward_only, summary_writer=None):
        ...
        outputs = session.fun(output_feed, input_feed)

        if summary_writer:
            summary_writer.add_summary(outputs[2], outputs[3])
        return outputs[:2]

    def step_beam(self, session, encoder_inputs, encoder_len, max_len=12, geneos=True):
        ...
        prev_tok = np.ones([beam_size], dtype='int32') * utils.ID_GO
        ...
        beam_att = np.zeros([self.batch_size, self.state_size * 2], dtype='float32')

        for i in range(max_len):
            input_feed[self.init_state] = prev_state
            input_feed[self.beam_tok] = prev_tok
            input_feed[self.prev_att] = beam_att
            output_feed = [self.beam_nxt_state[1], self.beam_logsoftmax, self.beam_nxt_state[0]]

            outputs = session.run(output_feed, input_feed)

            beam_att = outputs[0]
            tok_logsoftmax = np.asarray(outputs[1])
            tok_logsoftmax = tok_logsoftmax.reshape([beam_size, self.target_vocab_size])

            if not geneos:
                tok_logsoftmax[:, utils.ID_EOS] = -1e8

            tok_argsort = np.argsort(tok_logsoftmax, axis=1)[:, -beam_size:]
            tmp_arg0 = np.arrange(beam_size).reshape([beam_size, 1])
            ...
            prev_tok = tok_argsort[arg0, arg1]
            prev_state = outputs[2][arg0]
            score = tok_argsort_score[arg0, arg1]

            neos = neos[arg0] & (prev_tok != utils.ID_EOS)
    ...
    def add_pad(self, data, fixlen):
        data = map(lambda x: x + [utils.ID_PAD] * (fixlen - len(x)), data)
        data = list(data)
    ...
    def get_batch(self, data, bucket_id):
        ...
        # len(decoder_input) - 1 is the number of steps in the decoder
